...
| 5A ✅ | `feature/operanto-channel-foundation` (delivered 2026-08-02) | Provider-neutral adapter interface, ChannelInboundEvent store-then-process pipeline (atomic claim/retry/dead-letter), tenant-safe idempotency, normalization, consent model + keywords, delivery-status state machine, connection health, simulator on the canonical interface. See `docs/operanto-channel-foundation.md`. | 1, 4 |
| 5B | `feature/operanto-whatsapp-cloud` | WhatsApp Cloud connector: one Operanto-managed Meta app, per-org WABA, tenant-safe webhook routing, signature verification, encrypted credentials, template + 24h-window enforcement, explicit outbound send with full recheck chain, delivery receipts, feature flag. | 5A |
